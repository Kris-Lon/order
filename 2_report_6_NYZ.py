...
file = "/home/strateg-ai/order_scripts/xlsx_files/Ежемесячный_накопительный_отчет_по_количеству_и_сумме_заказов_НУЗов_от_{}.xlsx".format(today.isoformat())
...
for nyz_i in nyz:
    ...
    group by a.name, closed.count, cancel.count, inn12.summa, inn12.sups, inn12.vsego, pf.summa, pf.sups, pf.vsego order by a.name""".format(nyz_i)
    ...
    for idx, col in enumerate(df2):
        if df2[col].dtype in ('object', 'string_', 'unicode_') and df2[col].count() > 1:
    ...
    writer.save()
    send_mail(send_from="report@emsoft.ru", send_to=nyz[nyz_i], subject="Отчет для {}".format(df["Имя"].values[0]), text="""Ежемесячный накопительный отчет по количеству и сумме заказов НУЗов от {}. 
