...
file = "/home/strateg-ai/order_scripts/xlsx_files/Отчет_№5_Ежемесячный_отчет_с_заказами_содержащими_товары_с_НДС_0%_с_{}_по_{}.xlsx".format(first_day_last_month, lastMonth.isoformat())
...
ORDER BY order_s.order_number, agent.name""".format(first_day_last_month, lastMonth.isoformat())
...
worksheet = writer.sheets["Отчет№5"]
...
writer.save()
send_mail(send_from="report@emsoft.ru", send_to=["report@emsoft.ru", "kaloria@mail.ru"], subject="Отчет №5", text=" Ежемесячный отчет с заказами, содержащими товары с НДС 0% c {} по {} \n\nС уважением,\nООО \"Эмсофт\"\n+7 (495) 230-23-48\nreport@emsoft.ru\n".format(first_day_last_month, lastMonth.isoformat()), files=file)
