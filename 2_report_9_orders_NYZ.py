...
file = "/home/strateg-ai/order_scripts/xlsx_files/Ежемесячный_отчет_по_заказам_ПФ_живым_поставщикам_c_{}_по_{}.xlsx".format(first_day_last_month, lastMonth.isoformat())
...
ORDER BY temp.name, order_s.order_number""".format(nyz_i, first_day_last_month, lastMonth.isoformat())

    df2 = pd.read_sql_query(query,db_connection)
    #name_sheet = yesterday.isoformat()
    ...
    writer.save()
    send_mail(send_from="report@emsoft.ru", send_to=nyz[nyz_i], subject="Отчет для {}".format(df["Имя"].values[0]), text="""Ежемесячный отчет по заказам ПФ живым поставщикам c {} по {}. 
