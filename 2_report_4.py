...
file = "/home/strateg-ai/order_scripts/xlsx_files/Отчет_№4_Накопительный_отчет_по_заказам_с_01_01_2019_статус_которых_не_менялся_более_чем_90_дней_от_{}.xlsx".format(today.isoformat())
...
from order_s INNER JOIN contract ON contract.id = order_s.fk_contract_id 
...
and (now()::date - events.created_when::date)::int >= 90 
and order_state_event.order_state != 'ORDER_CLOSED' and order_state_event.order_state != 'ORDER_CANCELED' 
...
ORDER BY order_s.order_number, events.created_when"""

df2 = pd.read_sql_query(query,db_connection)
df2.to_excel(writer, encoding='utf8', sheet_name="Отчет№4", index=False, freeze_panes = (1,1))
worksheet = writer.sheets["Отчет№4"]
...
writer.save()
send_mail(send_from="report@emsoft.ru", send_to=["report@emsoft.ru", "kaloria@mail.ru"], subject="Отчет №4", text=" Накопительный отчет по заказам с 01.01.2019, статус которых не менялся более чем 90 дней от {} \n\nС уважением,\nООО \"Эмсофт\"\n+7 (495) 230-23-48\nreport@emsoft.ru\n".format(today.isoformat()), files=file)
