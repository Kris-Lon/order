...
file = "/home/strateg-ai/order_scripts/xlsx_files/Отчет_№7_Ежемесячный_отчет_за_предыдущие_7_дней_по_количеству_и_сумме_заказов_НУЗов_с_{}_по_{}.xlsx".format(first_day_last_month, lastMonth.isoformat())
...
left join 
    (select c.fk_customer_id, 
            ...
            count(os.id) as vsego from order_s os
    inner join contract c on c.id = os.fk_contract_id
...
AND os.created_when::date >= '{}' AND os.created_when::date <= '{}'
group by a.name, closed.count, cancel.count, inn12.summa, inn12.sups, inn12.vsego, pf.summa, pf.sups, pf.vsego order by a.name""".format(first_day_last_month, lastMonth.isoformat(), first_day_last_month, lastMonth.isoformat(), first_day_last_month, lastMonth.isoformat(), first_day_last_month, lastMonth.isoformat(), first_day_last_month, lastMonth.isoformat())

df2 = pd.read_sql_query(query,db_connection)
df2.to_excel(writer, encoding='utf8', sheet_name="Отчет№7", index=False, freeze_panes = (1,1))
...
writer.save()
send_mail(send_from="report@emsoft.ru", send_to=["report@emsoft.ru", "kaloria@mail.ru"], subject="Отчет №7", text=" Ежемесячный отчет за предыдущие 7 дней по количеству и сумме заказов НУЗов с {} по {} \n\nС уважением,\nООО \"Эмсофт\"\n+7 (495) 230-23-48\nreport@emsoft.ru\n".format(first_day_last_month, lastMonth.isoformat()), files=file)
