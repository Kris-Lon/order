...
file = "/home/strateg-ai/order_scripts/xlsx_files/Отчет_№16_1_Детализация_отчет_по_незакрытым_заказам_от_{}.xlsx".format(today.isoformat())
...
ORDER BY agent.name, order_s.order_number"""

df2 = pd.read_sql_query(query,db_connection)
df2.to_excel(writer, encoding='utf8', sheet_name="Отчет №16.1", index=False, freeze_panes = (1,1))
worksheet = writer.sheets["Отчет №16.1"]
...
writer.save()
send_mail(send_from="report@emsoft.ru", send_to=["report@emsoft.ru", "kaloria@mail.ru"], subject="Отчет №16.1", text="Детализация отчета по незакрытым заказам от {} \n\nС уважением,\nООО \"Эмсофт\"\n+7 (495) 230-23-48\nreport@emsoft.ru\n".format(today.isoformat()), files=file)
